...
class videoa(QtCore.QThread):
    ...
    #colorget相关的变量，用于hsv空间处理图像，实际上并不用
    H1 = 110
    S1 = 50
    V1 = 50
    H2 = 130
    ...
    def imageProcess(self,frame):
        # im = cv2.imread(frame)
        listInfo = [0, 0, 0, 0]#保存图像处理完的信息
        im = frame
        imageColor = im.copy()#复制图像

        result1 = cv2.blur(im,(5,5)) # # 用低通滤波平滑图像dst = cv2.boxFilter(im, -1, (5, 5))  ;
        # 0 是指根据窗口大小（5,5）来计算高斯函数标准差
        # result1 = cv2.GaussianBlur(im, (5, 5), 0)

        imgray = cv2.cvtColor(result1, cv2.COLOR_BGR2GRAY)#图像灰度处理

        ret, thresh = cv2.threshold(imgray, 127, 255, 0)#二值化图像

        contours, hierarchy = cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)#检测图像轮廓

        if contours:#判断是否检测到图像轮廓
    ...
    def select(self,imafotion,sortInfo):
        ...
        if(sortInfo == 1):  #按大小
            ...
            if (imafotion[1] <1.4):  # 圆度1
                print "正圆"
                step = -80
            elif (imafotion[1] >=1.4 and imafotion[1] <5):  # 圆度2
                print "圆"
                step = -15
        ...
        while success and self.stopa > 0:
            ...
            if(self.stopa == 2):
                ...
                if(loopNumber>20):
                    ...
                    step = self.select(listInfo, self.sortInfo)
                    if(loopNumber>100):
                        loopNumber = 20
                loopNumber += 1
                while step:
                ...
                # newQimage = qImage.scaledToWidth(371) #使用图像宽度匹配
                newQimage = qImage.scaledToHeight(231)  #使用图像高度匹配
            ...
            self.emit(QtCore.SIGNAL("layoutLoadImage"), newQimage)#发送图像信号
        cv2Video.release()
        print "关闭摄像头线程"
